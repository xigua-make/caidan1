...
export async function downloadImage({
  ...
  const processDownload = async () => {
    ...
    ctx.fillText('小瓜拼豆生成器', titleStartX, mainTitleY);
    ...
    if (includeStats && colorCounts) {
      ...
      const statsWatermarkText = '图纸来源：@小瓜拼豆生成器';
    ...
    // iOS 大图分块下载：检测是否需要分块
    const IOS_SAFE_CANVAS_AREA = 4000000; // iOS 安全 canvas 面积（约 4MP）
    const estimatedTotalArea = downloadWidth * downloadHeight;
    const needsSplit = isIOS() && estimatedTotalArea > IOS_SAFE_CANVAS_AREA;

    if (needsSplit) {
      // 计算分块数量（按面积计算需要分成几块）
      const splitCount = Math.ceil(estimatedTotalArea / IOS_SAFE_CANVAS_AREA);
      const splitCols = Math.ceil(Math.sqrt(splitCount));
      const splitRows = Math.ceil(splitCount / splitCols);
      
      console.log(`iOS 大图分块下载：${splitRows}行 x ${splitCols}列，共 ${splitRows * splitCols} 块`);
      
      // 计算每块的实际像素范围（只分网格区域，标题和统计在每块都显示）
      const gridStartX = extraLeftMargin + axisLabelSize;
      const gridStartY = titleBarHeight + extraTopMargin + axisLabelSize;
      const gridEndX = gridStartX + gridWidth;
      const gridEndY = gridStartY + gridHeight;
      
      const blockPixelWidth = Math.ceil(gridWidth / splitCols);
      const blockPixelHeight = Math.ceil(gridHeight / splitRows);
      
      // 计算每块的网格范围
      const blockGridCols = Math.ceil(N / splitCols);
      const blockGridRows = Math.ceil(M / splitRows);
      
      // 依次下载每个分块
      for (let row = 0; row < splitRows; row++) {
        for (let col = 0; col < splitCols; col++) {
          // 计算当前块的网格范围
          const startGridCol = col * blockGridCols;
          const endGridCol = Math.min((col + 1) * blockGridCols, N);
          const startGridRow = row * blockGridRows;
          const endGridRow = Math.min((row + 1) * blockGridRows, M);
          
          const currentBlockGridWidth = endGridCol - startGridCol;
          const currentBlockGridHeight = endGridRow - startGridRow;
          
          if (currentBlockGridWidth <= 0 || currentBlockGridHeight <= 0) continue;
          
          // 创建分块 canvas
          const blockCanvas = document.createElement('canvas');
          const blockWidth = currentBlockGridWidth * downloadCellSize + (axisLabelSize * 2) + extraLeftMargin + extraRightMargin;
          const blockHeight = titleBarHeight + currentBlockGridHeight * downloadCellSize + (axisLabelSize * 2) + statsHeight + extraTopMargin + extraBottomMargin + xiaohongshuAreaHeight;
          ...
          // 复制标题栏
          blockCtx.drawImage(downloadCanvas, 0, 0, downloadWidth, titleBarHeight, 0, 0, blockWidth, titleBarHeight);
          
          // 绘制坐标轴和网格内容（需要重新计算坐标）
          const blockGridStartX = startGridCol * downloadCellSize;
          const blockGridStartY = startGridRow * downloadCellSize;
          
          // 从原 canvas 截取对应区域的网格部分
          const sourceGridX = gridStartX + blockGridStartX;
          const sourceGridY = gridStartY + blockGridStartY;
          const sourceGridWidth = currentBlockGridWidth * downloadCellSize;
          const sourceGridHeight = currentBlockGridHeight * downloadCellSize;
          
          // 绘制网格区域到分块 canvas
          const destGridX = extraLeftMargin + axisLabelSize;
          const destGridY = titleBarHeight + extraTopMargin + axisLabelSize;
          
          blockCtx.drawImage(
            downloadCanvas,
            sourceGridX, sourceGridY, sourceGridWidth, sourceGridHeight,
            destGridX, destGridY, sourceGridWidth, sourceGridHeight
          );
          
          // 绘制坐标标签（四边）
          if (showCoordinates) {
            blockCtx.font = `bold ${Math.max(10, Math.floor(downloadCellSize * 0.5))}px sans-serif`;
            ...
            // 上边坐标
            for (let i = startGridCol; i < endGridCol; i++) {
              const cellX = destGridX + (i - startGridCol) * downloadCellSize;
              const cellY = titleBarHeight + extraTopMargin;
            ...
            for (let i = startGridCol; i < endGridCol; i++) {
              const cellX = destGridX + (i - startGridCol) * downloadCellSize;
              const cellY = titleBarHeight + extraTopMargin + axisLabelSize + currentBlockGridHeight * downloadCellSize;
              blockCtx.fillStyle = '#E3F2FD';
              blockCtx.fillRect(cellX, cellY, downloadCellSize, axisLabelSize);
              blockCtx.strokeStyle = '#333333';
              blockCtx.lineWidth = 1;
            ...
            // 左边坐标
            for (let j = startGridRow; j < endGridRow; j++) {
              const cellX = extraLeftMargin;
              const cellY = titleBarHeight + extraTopMargin + axisLabelSize + (j - startGridRow) * downloadCellSize;
              blockCtx.fillStyle = '#E3F2FD';
              ...
              blockCtx.fillStyle = '#333333';
              blockCtx.textBaseline = 'middle';
              blockCtx.fillText((j + 1).toString(), cellX + axisLabelSize / 2, cellY + downloadCellSize / 2);
            }
            
            // 右边坐标
            for (let j = startGridRow; j < endGridRow; j++) {
              const cellX = destGridX + currentBlockGridWidth * downloadCellSize;
              const cellY = titleBarHeight + extraTopMargin + axisLabelSize + (j - startGridRow) * downloadCellSize;
              blockCtx.fillStyle = '#E3F2FD';
              blockCtx.fillRect(cellX, cellY, axisLabelSize, downloadCellSize);
              blockCtx.strokeStyle = '#333333';
          ...
          // 复制统计区域（只在最后一个分块显示）
          if (row === splitRows - 1 && col === splitCols - 1 && statsHeight > 0) {
            const statsY = titleBarHeight + extraTopMargin + currentBlockGridHeight * downloadCellSize + (axisLabelSize * 2);
            const sourceStatsY = titleBarHeight + extraTopMargin + gridHeight + (axisLabelSize * 2);
            
            blockCtx.drawImage(
              downloadCanvas,
              0, sourceStatsY, downloadWidth, statsHeight + xiaohongshuAreaHeight,
              0, statsY, blockWidth, statsHeight + xiaohongshuAreaHeight
            );
          }
          ...
          blockLink.download = showCellNumbers
            ? `bead-grid-${N}x${M}_part${row * splitCols + col + 1}-${splitRows * splitCols}-keys-palette_${selectedColorSystem}.png`
            : `bead-grid-${N}x${M}_part${row * splitCols + col + 1}-${splitRows * splitCols}-pixel-palette_${selectedColorSystem}.png`;
          blockLink.href = blockDataURL;
          document.body.appendChild(blockLink);
          blockLink.click();
          document.body.removeChild(blockLink);
          
          console.log(`分块 ${row * splitCols + col + 1}/${splitRows * splitCols} 已下载`);
